...
void eql(qk_tap_dance_state_t *state, void *user_data)
{
  if (state->count > 2) {
    ...
    double_max(state->count, SHIFT, KC_0);
  }
  reset_tap_dance(state);
}
...
void greater(qk_tap_dance_state_t *state, void *user_data)
{
  if (state->count > 2) {
    ...
    double_max(state->count, SHIFT, KC_NUBS);
  }
  reset_tap_dance(state);
}
...
void lesser(qk_tap_dance_state_t *state, void *user_data)
{
  if (state->count > 2) {
    ...
    double_max(state->count, NOSHIFT, KC_NUBS);
  }
  reset_tap_dance(state);
}
...
{
  tap_pair(state, S_ALWAYS, KC_2, KC_2, 0, 0);
}

void grave(qk_tap_dance_state_t *state, void *user_data)
{
  tap_pair(state, S_ALWAYS, KC_EQL, KC_EQL, 0, 0);
}

void lbrace(qk_tap_dance_state_t *state, void *user_data)
{
  tap_pair(state, S_NEVER, KC_8, KC_9, KC_RALT, 0);
}
...
{
  tap_pair(state, S_NEVER, KC_7, KC_0, KC_RALT, 0);
}
...
{
  tap_pair(state, S_ALWAYS, KC_8, KC_9, KC_LCTL, 0);  // Why the left CTRL?
}
...
{ 
  tap_pair(state, S_NEVER, KC_NUHS, KC_NUHS, 0, 0);  
}

void rangle(qk_tap_dance_state_t *state, void *user_data)
{
  tap_pair(state, S_ALWAYS, KC_NUBS, KC_NUBS, 0, CLOSE); // This is probably just >> and not <>
}

void rbrace(qk_tap_dance_state_t *state, void *user_data)
{
  tap_pair(state, S_NEVER, KC_8, KC_9, KC_RALT, CLOSE);
}
...
{
  tap_pair(state, S_NEVER, KC_7, KC_0, KC_RALT, CLOSE);
}
...
{
  tap_pair(state, S_ALWAYS, KC_8, KC_9, KC_LCTL, CLOSE);  // Why the left CTRL?
}
...
qk_tap_dance_action_t tap_dance_actions[] = {
 ...
 ,[_LCBR] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, lcurly, lcurly_reset)
