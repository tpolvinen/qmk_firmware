...
//
//   cd qmk_firmware
//   make planck/rev4:tatunordicpc:clean (good practice before flashing)
//   Reset keyboard or press hw reset button on base (hole).
//   make planck/rev4:tatunordicpc:dfu-util
//
//
...
//
//   For PCs (Linux) with Swedish/Finnish keyboard layout
//
//
...
#define LT_BSLS LT  (_MOUSE,  NO_BSLS)      // see process_record_user() for extended handling
...
#ifdef THUMB_0
#define LT_EQL  LT  (_ADJUST, NO_EQL)
#else
...
bool process_record_user(uint16_t keycode, keyrecord_t *record)
{
  switch (keycode) {
    ...
    case SM_CIRC:
      // GUI_T(S(KC_6))
      mt_shift(record, KC_LGUI, 0, KC_6); // TODO: How to write LSFT(KC_RBRC) here? See symbol_guifn_layout.h
      break;
    case SM_DLR:
      // SFT_T(S(KC_4))
